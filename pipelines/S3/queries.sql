...
SELECT SUM(ausgaben_in_mio_chf) as ausgaben_in_mio_chf,
...
FROM staatsausgaben_nach_aufgabenbereichen_cofog
WHERE jahr=2018 AND aufgabenbereich_cofog_grobgliederung = 'Umweltschutz'
...
SELECT jahr, SUM(prozent_der_gesamtausgaben) as prozent_der_gesamtausgaben_fur_sicherheit
FROM staatsausgaben_nach_aufgabenbereichen_cofog
WHERE LOWER(aufgabenbereich_cofog_feingliederung) LIKE '%sicherheit%'
...
FROM staatsausgaben_nach_aufgabenbereichen_cofog
WHERE LOWER(aufgabenbereich_cofog_feingliederung) LIKE '%kommunikation%'
...
FROM staatsausgaben_nach_aufgabenbereichen_cofog
WHERE aufgabenbereich_cofog_grobgliederung = 'Gesundheitswesen'
    AND institutioneller_sektor = 'Staat'
    AND jahr <= 2020
    AND jahr >= 2010
GROUP BY jahr
...
FROM staatsausgaben_nach_aufgabenbereichen_cofog
WHERE aufgabenbereich_cofog_grobgliederung = 'Bildungswesen'
    AND jahr = 2020
    AND institutioneller_sektor = 'Kantone'
